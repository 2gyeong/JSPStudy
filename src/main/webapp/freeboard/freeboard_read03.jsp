...
<body>
	...
	<%
		...
		// String 형식으로 넘어온다. Integer.parseInt();
...
// 방법 2 PreparedStatement 객체를 사용해서 처리
			// 매번 컴파일 하지 않기 때문에 처리 속도가 빠름
		sql = "select * from freeboard where id = ?";
		pstmt = conn.prepareStatement(sql); // pstmt 객체 생성 시 sql문을 넣는다.
		...
		// Statement 객체의 select/insert/delete 쿼리인 경우 executeUpdate(sql) 를 실행
		// Statement 객체의 select 쿼리인 경우 executeQuery(sql) 를 실행, Resultset 객체로 리턴
		
		stmt = conn.createStatement();
		stmt.executeUpdate(sql);
		
/* 	방법 1	// 읽기 페이지 마지막에서 글 조회수를 늘려준다.
		sql = "update freeboard set readcount= readcount +1 where id = " + id ;
	...
	<table width='600' cellspacing='0' cellpadding='2' align='center'>
	   ...
	   <div align='left'><font size='4'></div> 글 제목 : <%= rs.getString("subject") %></td>
	   </tr>
   ...
   <table width="600" border="0" cellpadding="0" cellspacing="5">
	<tr> 
		...
		<td width="70" align="right"><A href="freeboard_rwrite.jsp?id=<% request.getParameter("id"); %>&page=<% request.getParameter("page"); %>"></A> <img src="image/reply.jpg" border=0></A></td>
